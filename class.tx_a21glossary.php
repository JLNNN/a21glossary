		...
		if (!$this->count['used']) {
			...
			foreach ($items as $item) {
				...
				if (!\TYPO3\CMS\Core\Utility\GeneralUtility::inList($conf['excludeTypes'], $item['shorttype'])) {
					...
					} elseif (\TYPO3\CMS\Core\Utility\GeneralUtility::inList($conf['linkToGlossary'], $item['shorttype']) && count($conf['typolink.'])) {
						$generateLink = 1;

						if (strlen($conf['linkOnlyIfNotEmpty'])) {
							$linkOnlyIfNotEmpty = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',',$conf['linkOnlyIfNotEmpty']);
			...
			if ($this->count['local']) {
				...
				$this->parseObj = new \TYPO3\CMS\Core\Html\HtmlParser();
